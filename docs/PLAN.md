...
Production builds from `main` at https://13parbon.org.uk, with a preview URL on every PR. `vercel.json` rewrites every route to `index.html` for client-side routing. Netlify config is kept in the repo as a fallback.
