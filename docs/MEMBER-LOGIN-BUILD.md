...
| Date | Step | |
|---|---|---|
| 2026-09-17 | 4 | The bucket's contents checked where the browser cannot reach. Every metadata check ran on the machine being defended against, and the signed PUT pins what an upload claims rather than what its bytes are. The object is read back by the function now and put through the same rules; anything carrying metadata comes out of the bucket and no row is written, so a refused photograph is nowhere. Proven on the real bucket the same day: sign at 12:36:49, read-back at 12:36:51, and the upload only finished because the second said yes. 775 → 781 tests. |
