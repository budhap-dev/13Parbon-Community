...
describe('EventPage', () => {
  it('shows the event, its countdown, registration and volunteer roles', async () => {
    ...
    expect(screen.getByLabelText('37 days to go')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Register the family' })).toHaveAttribute('href', '/login')
  ...
  it('hides registration and the countdown for past events', async () => {
    ...
    expect(screen.queryByLabelText(/days to go/)).not.toBeInTheDocument()
